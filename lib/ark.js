...
(function() {
  var Ark, CoffeeScript, Crypto, Eco, Fibers, FileSystem, Future, Path, Util, base64, beautify, code, compile_coffeescript, dependencies, error, glob, index, inspect, make_synchronous, manifest, md5, minify, read, readdir, render, run_as_fiber, stat, warn,
  ...
  Ark = {
    manifest: function(options) {
      ...
      return JSON.stringify(manifest(options), null, 2);
    },
    "package": function(options) {
      var _ref;
      if ((_ref = options.manifest) == null) {
        options.manifest = manifest(options);
      }
      if (options.minify) {
        return minify(code(index(options.manifest)));
      } else {
        return beautify(code(index(options.manifest)));
      }
    }
  };
